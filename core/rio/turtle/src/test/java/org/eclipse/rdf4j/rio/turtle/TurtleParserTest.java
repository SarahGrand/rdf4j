...
public class TurtleParserTest extends AbstractParserTest {
	...
	private final Statement simpleSPOStatement = vf.createStatement(vf.createIRI("http://example/s"), vf.createIRI("http://example/p"), vf.createIRI("http://example/o"));
	private final Triple simpleSPOTriple = vf.createTriple(vf.createIRI("http://example/s"), vf.createIRI("http://example/p"), vf.createIRI("http://example/o"));
...
//	}
	...
	private Statement simpleExampleStatement(String s, String p, String o) {
		return vf.createStatement(vf.createIRI("http://example/" + s), vf.createIRI("http://example/" + p), vf.createIRI("http://example/" + o));
	}

	private Triple simpleExampleTriple(String s, String p, String o) {
		return vf.createTriple(vf.createIRI("http://example/" + s), vf.createIRI("http://example/" + p), vf.createIRI("http://example/" + o));
	}
	...
	public void testNestedTripleTerms() throws IOException {
		...
		try {
			...
			assertEquals(stmt2, vf.createStatement(vf.createIRI("http://example/a"), RDF.REIFIES, simpleExampleTriple("s1", "p1", "o1")));
			assertEquals(stmt3, vf.createStatement(vf.createIRI("http://example/r"), RDF.REIFIES,
					vf.createTriple(vf.createIRI("http://example/23"), RDF.REIFIES, simpleExampleTriple("s3", "p3", "o3"))));
		} catch (RDFParseException e) {
			fail("parse error on correct data: " + e.getMessage());
		}
	}
	...
	public void testSubjectReifiedTriple() throws IOException {
		...
		try {
			...
			assertTrue(model.contains(reifier, vf.createIRI("http://example/q"), vf.createLiteral(BigInteger.valueOf(123))));
		} catch (RDFParseException e) {
			fail("parse error on correct data: " + e.getMessage());
		}
	}
